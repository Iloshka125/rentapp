...
**Что делает:**  
1. Извлекает из `req.body` поля `name`, `description`, `userId`, `category`, `photo`, `price`, `startdate`, `enddate` и опционально `rating`.  
...
3. Если переданы поля `startdate` и `enddate`, проверяет их корректность:
   - Даты должны быть в валидном формате
...
- `name`, `description`, `category`, `photo` — основная информация о товаре.  
- `images` — дополнительные изображения товара (JSON строка с массивом путей к файлам).  
- `price` — цена (в модели как FLOAT).  
- `rating` — средний рейтинг товара (агрегат из отзывов).
- `status` — статус товара (по умолчанию "available").
...
- `isreview` — флаг возможности оставить отзыв (по умолчанию `false`). Отзывы разрешены для аренд со статусом `active` (принятый заказ).
...
- **idProduct** (INTEGER, PK) - уникальный идентификатор товара
- **userId** (INTEGER, FK) - ссылка на владельца товара (таблица User)
- **name** (STRING) - название товара
- **description** (TEXT) - описание товара
- **category** (STRING) - категория товара
- **photo** (STRING) - путь к основному изображению товара
- **images** (TEXT) - JSON строка с массивом путей к дополнительным изображениям
- **price** (FLOAT) - цена за аренду
- **rating** (FLOAT) - рейтинг товара (по умолчанию 0)
- **status** (STRING) - статус товара ('available', 'unavailable', 'deleted')
...
- **idUser** (INTEGER, PK) - уникальный идентификатор пользователя
...
- **idRent** (INTEGER, PK) - уникальный идентификатор аренды
- **idUser** (INTEGER, FK) - ссылка на арендатора (таблица User)
- **idProduct** (INTEGER, FK) - ссылка на арендуемый товар (таблица Product)
- **status** (STRING) - статус аренды
- **dataStart** (DATE) - дата начала аренды
- **dataEnd** (DATE) - дата окончания аренды
- **isreview** (BOOLEAN) - флаг возможности оставить отзыв

### Таблица Review
Таблица отзывов о товарах:

- **idReview** (INTEGER, PK) - уникальный идентификатор отзыва
- **idUser** (INTEGER, FK) - ссылка на автора отзыва (таблица User)
- **idProduct** (INTEGER, FK) - ссылка на товар (таблица Product)
- **rate** (FLOAT) - оценка (количество звезд)
- **comment** (TEXT) - текстовый комментарий
- **uploadDate** (DATE) - дата создания отзыва

### Таблица Favourite
Таблица избранных товаров:

- **idFavourite** (INTEGER, PK) - уникальный идентификатор записи
- **userId** (INTEGER, FK) - ссылка на пользователя (таблица User)
- **idProduct** (INTEGER, FK) - ссылка на товар (таблица Product)

### Таблица Notification
Таблица уведомлений:

- **idNotification** (INTEGER, PK) - уникальный идентификатор уведомления
- **userId** (INTEGER, FK) - ссылка на пользователя (таблица User)
